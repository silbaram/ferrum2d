...
#[test]
fn weld_joint_non_finite_mass_matrix_uses_safe_fallback() {
    let mut world = World::default();
    let anchor = world.spawn_entity();
    world.set_transform(anchor, Transform2D { x: 0.0, y: 0.0 });
    world.set_rotation(anchor, Rotation2D { radians: 0.0 });
    ...
    let body = world.spawn_entity();
    let initial_transform = Transform2D { x: 4.0, y: 2.0 };
    let initial_rotation = Rotation2D { radians: 0.5 };
    world.set_transform(body, initial_transform);
    world.set_rotation(body, initial_rotation);
    let mut extreme_body = RigidBody::dynamic(1.0);
    extreme_body.inertia = f32::MIN_POSITIVE;
    extreme_body.inverse_inertia = f32::MAX;
    world.set_rigid_body(body, extreme_body);
    world.add_weld_joint(
        WeldJoint::new(anchor, body)
            .with_local_anchor_b(2.0, 2.0)
            .with_stiffness(1.0)
            .with_angular_stiffness(1.0),
    );

...
function renderBootstrapError(error: unknown): void {
  console.error("Ferrum2D Platformer failed", error);
  ...
  title.textContent = "Ferrum2D Platformer";
...
function createShell(): {
  ...
  const kinematicValue = document.createElement("dd");
  ...
  appendMetric(metrics, "kinematic", kinematicValue);
  ...
  try {
    ...
    const runtime = await createFerrumRuntime({
      ...
      onFrame: ({ frame, rendererStats, fps }) => {
        ...
        shell.kinematicValue.textContent = `${frame.physics.kinematicMoves}/${frame.physics.kinematicHits}`;

...
- AABB/circle collider: 기존 `AabbCollider`와 별도로 `CircleCollider`를 지원하고, current-frame broadphase pair는 circle bounds를 proxy로 사용한 뒤 precise overlap으로 확정한다.
- AABB/circle contact: `AabbContact`와 `CollisionContact`가 AABB/AABB, AABB/circle, circle/circle contact normal과 penetration을 제공한다. contact normal은 pair의 `a`에서 `b` 방향이며, `a`를 밀어낼 때는 `-normal * penetration`을 적용한다.
- Physics debug line: `CollisionSystem::build_broadphase_debug_lines(...)`는 current-frame broadphase AABB proxy bounds를 사각형 line primitive로 만들고, `build_contact_debug_lines(...)`는 contact normal을 line primitive로 만든다. `build_physics_debug_lines(...)`는 두 결과를 같은 `PhysicsDebugLine` f32 buffer에 담는다. `Engine.set_physics_debug_lines_enabled(...)`로 opt-in 생성하고, Wasm 경계는 `decodePhysicsDebugLines(...)`를 사용한다. `FrameState.physicsDebugLineBuffer`는 항상 제공하되 기본값은 빈 buffer이고, decoded `FrameState.physicsDebugLines`는 `includePhysicsDebugLines` opt-in이다. `WebGL2Renderer.renderPhysicsDebugLines(...)`와 `createFerrumRuntime(..., { physicsDebugLines: true })`는 같은 buffer를 debug line pass로 그린다.
- Query API: `point_query(...)`, `aabb_query(...)`, `circle_query(...)`, `shape_query(...)`, `shape_cast(...)`, `shape_cast_all(...)`, `raycast(...)`, `raycast_all(...)`이 `CollisionMask` 기반으로 world collider를 조회한다. `CollisionQueryShape`는 AABB와 circle query/cast shape를 지원한다. Shape cast hit의 point는 cast shape reference point가 impact 시점에 도달한 world position이고, normal은 맞은 collider surface normal이다.
...
- Generic World component setter: `World::set_transform(...)`, `set_velocity(...)`, `set_aabb_collider(...)`, `set_circle_collider(...)`, `set_collision_filter(...)`로 shooter prefab 경로 없이도 physics entity를 구성할 수 있다.
- Tilemap collision candidate range/merge: `collision: true` layer 전체를 매 entity마다 순회하지 않고, 인접 solid tile run을 merged AABB obstacle로 캐시한 뒤 collider AABB 또는 swept path가 겹치는 obstacle만 검사한다.
...
- Collision event lifecycle/payload: `CollisionEventTracker`가 current-frame AABB pair의 enter/stay/exit를 추적한다. Shooter gameplay 충돌처럼 충돌 처리 직후 despawn되는 pair는 `hit` event로 기록한다. Bullet/enemy hit event는 bullet damage를 payload로 담고, 다른 event는 damage `0`을 사용한다. Wasm 경계는 `CollisionEvent` u32 buffer와 `decodeCollisionEvents(...)`를 사용하며, `FrameState.collisionEventBuffer`는 항상 제공하고 decoded `FrameState.collisionEvents`는 `includeCollisionEvents` opt-in이다.

## 제외 범위

- fixed timestep 기본값 강제 전환
- substep
- capsule/polygon collider
- swept circle collider
...
- AABB/circle contact normal/penetration
- contact debug line normal
- broadphase debug bounds
- point/AABB/circle/shape/raycast query
- AABB/circle shape cast
- kinematic move-and-slide stop/slide/trigger ignore
- tilemap candidate range와 merged AABB obstacle
- tilemap swept AABB contact와 kinematic tile stop
- fixed timestep accumulator clamp/backlog drop
- physics counter 누적
- collision enter/stay/exit/hit lifecycle과 damage payload
- Top-down Shooter fast bullet collision scoring
- TypeScript collision event buffer decode와 public API type export
- TypeScript physics debug line buffer decode와 public API type export

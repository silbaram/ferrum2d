...
이 문서는 `v0.1.0` MVP 개발 완료 이후 **상용제품 기능 개발** 작업을 실제 진행 순서대로 정리한다. 기준일은 2026-05-19이며, 안정화된 기반 위에서 사용자-facing 제품 기능과 배포 품질을 단계적으로 확장한다.

## 목표

- 완료된 MVP 수직 슬라이스를 깨지 않고 public API, 실행 품질, 상용제품 기능을 함께 발전시킨다.
...
- Game Spec `enemies.presets`와 `enemies.waves`를 추가했다.
- TypeScript가 enemy preset 이름을 검증하고 Rust에는 wave duration, spawn interval, enemy count, enemy speed, behavior/spawn pattern code, health, score reward 숫자만 전달한다.
- Rust `ShooterScene`이 active wave, elapsed time, spawn count, spawn timer를 소유하고 마지막 wave 이후 첫 wave부터 반복한다.
...
- Game Spec `audio.masterVolume`, `audio.sfxVolume`, `audio.events.*.volume`, `audio.events.*.pitch`를 추가했다.
- `AudioManager.configure(...)`와 `AudioManager.unlock()`을 추가하고 `BrowserPlatformHost`에서 audio bus 설정과 unlock을 노출했다.
- Top-down Shooter 예제가 첫 key/pointer 입력에서 audio unlock을 시도한다.
...
- 정적 tile layer 렌더링 (완료)
- tile id -> UV 매핑 (완료)
- collision layer의 AABB 변환 (완료)

완료 내용:

- Game Spec `tilemap.tiles`와 `tilemap.layers`를 추가했다.
- TypeScript가 tile id, atlas frame 참조, color, layer dimension, row-major data 길이와 tile id 참조를 검증한다.
- Rust `Tilemap`이 entity 생성 없이 static tile render command를 생성하고 기존 `SpriteRenderCommand` ABI를 유지한다.
- Rust `Tilemap`이 `collision: true` layer의 양수 tile을 player/enemy 이동을 막는 정적 AABB 장애물로 해석한다.
- Top-down Shooter 예제 `game.json`에 placeholder texture id `0` 기반 arena floor tile layer와 obstacle collision layer를 적용했다.

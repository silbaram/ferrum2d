  ...
  "content": {
    "localization": {
      ...
      "locales": {
        "en": {
          "strings": {
            "intro.ready": "Ready to hold the arena?",
            "intro.choice.start": "Start patrol",
            "intro.done": "Keep moving and watch the corners."
          }
        },
        "ko": {
          "strings": {
            "intro.ready": "아레나를 지킬 준비가 됐나요?",
            "intro.choice.start": "순찰 시작",
            "intro.done": "계속 움직이고 모서리를 살피세요."
          }
        }
      }
    },
        ...
        "intro": {
          ...
          "nodes": {
            "start": {
              "speaker": "Guide",
              "text": "intro.ready",
              "choices": [{ "id": "start", "label": "intro.choice.start", "to": "done" }]
            },
    ...
    "cutscenes": {
      "intro": {
        "id": "intro",
        "commands": [
          { "kind": "dialogue", "graphId": "intro", "nodeId": "start", "durationSeconds": 1.5 },
...
모든 필드는 선택 사항이다. 누락된 값은 `packages/ferrum-web/src/gameSpec.ts`의 기본값으로 채운다. 위 예시는 player에 state animation을 적용하고 bullet과 tilemap은 atlas frame metadata를 통해 texture/UV/size를 받는 형태다. 실제 예제 `game.json`은 10x6 tilemap에 `floor`, `panel`, `accent`, `block` tile과 `collision: true` obstacle layer를 포함한다. `prefabs.player`, `prefabs.enemy`, `prefabs.bullet`은 같은 animation 구조, atlas frame binding, AABB/circle/capsule/oriented-box/convex-polygon collider metadata를 지원한다. `content`는 localization, dialogue graph, cutscene sequence authoring 데이터를 검증하는 namespace이며, runtime 연결은 `createShooterContentRuntimeOptions(resolved.content)`가 `createFerrumRuntime({ localization, dialogue, cutscene })` option fragment로 변환한다. `sceneComposition`과 `behaviorRecipes`는 메인 Shooter Game Spec top-level에 추가하지 않는다. consumer template의 `public/scene-authoring.json` 같은 별도 artifact를 `resolveSceneAuthoringDocument(...)`로 검증하고, 게임별 adapter가 낮은 빈도 apply 경로에서 spawn/entity binding을 수행한다.
...
| `content.localization` | object | omitted | `resolveLocalizationDocument(...)`로 검증되는 localization document |
| `content.dialogue.graphs.*` | object | omitted | `resolveDialogueGraph(...)`로 검증되는 dialogue graph |
| `content.cutscenes.*` | object | omitted | `resolveCutsceneSequenceSpec(...)`로 검증되는 cutscene sequence |

`createShooterContentRuntimeOptions(...)`의 자동 선택 정책은 단순하다. localization은 존재하면 연결한다. dialogue graph와 cutscene은 명시 id를 넘기거나 후보가 정확히 하나일 때만 선택하며, 후보가 여러 개이면 자동 선택하지 않는다. cutscene dialogue command가 `graphId`/`nodeId`만 갖고 `text`가 없으면 해당 dialogue graph node의 `text`/`speaker`로 hydrate하고, localization이 있으면 cutscene dialogue text mode 기본값을 `localizationKey`로 둔다.
...
`examples/topdown-shooter/public/authored-behavior.variant.json`은 메인 `game.json`에 새 namespace를 넣지 않고, Top-down Shooter가 gameplay authoring 데이터를 어떻게 묶는지 보여주는 예제용 variant다. create-game consumer template의 `public/scene-authoring.json`은 같은 방향을 따르는 별도 `ferrum2d.consumer.scene-authoring` envelope이며, public package의 `resolveSceneAuthoringDocument(...)`가 format/version, `sceneComposition`, `behaviorRecipes`, optional `ids`, optional binding plan을 검증한다.
...
- `expected.agentAttachment`: selected placed id가 어떤 behavior profile과 runtime command target으로 컴파일되어야 하는지 고정하는 dry-run drift check. agent가 배치 객체 id를 받아 `behaviorRecipes`와 binding을 수정했을 때, command가 다른 instance id로 새지 않는지 확인한다.
...
두 명령은 같은 semantic validator를 실행한다. `resolveShooterGameSpec(...)`, `resolveGameplayBehaviorRuntimeIds(...)`, `resolveSceneCompositionSpec(...)`, `dryRunSceneBehaviorRecipes(...)`, `resolveBehaviorStateMachineDocument(...)`, `createBehaviorStateMachineRuntimeInstallPlan(...)`를 public package build에서 실행하고, `ids.actions`, `semantics.browserPlacement`, replay manifest 연결도 확인한다. `pnpm validate:gameplay-authoring:report`는 같은 입력에서 `gameplayAuthoringDryRun.agentAttachment`를 만들어 `expected.agentAttachment`와 비교하므로, selected id 기반 behavior attachment drift도 machine-actionable report로 잡는다. 즉 variant는 browser runtime에서 바로 로드되는 메인 Game Spec은 아니지만, agent가 prefab/behavior/FSM/replay 계약을 한 파일에서 patch하고 검증할 수 있는 authoring artifact다. 현재 variant의 FSM은 state transition telemetry를 검증하는 범위이며, state entry마다 behavior profile을 자동 apply/clear하는 runtime을 의미하지 않는다.

Production Top-down Shooter build도 이 variant JSON을 asset manifest로 로드하고, 같은 public authoring API 검증 결과를 `window.ferrumTopdownAuthoredBehaviorVariant` summary로 노출한다. 이것이 현재 public Pages demo의 runtime extensibility 노출 기본값이다. 기본 demo UI에는 별도 profile selector를 추가하지 않고, variant runtime 적용은 `authoredBehaviorVariantApply=true` query가 켜진 경우에만 실행한다.

노출 범위는 의도적으로 다음 경계에 묶는다.

- 기본 Pages/demo 진입은 `game.json` 기반 built-in Shooter runtime이다.
- `window.ferrumTopdownAuthoredBehaviorVariant` summary는 smoke/agent가 loaded variant, replay scenario, expected state id, runtime apply 가능성을 읽기 위한 낮은 빈도 inspection surface다.
- `authoredBehaviorVariantApply=true` query는 browser smoke와 manual QA가 authored SceneComposition/Behavior Recipe/FSM 설치를 실제 runtime에 적용하기 위한 opt-in이다.
- `ferrumTopdownAuthoredBehaviorStart()`, `ferrumTopdownAuthoredBehaviorApplyCurrentStateCommands()`, `ferrumTopdownAuthoredBehaviorResetAndReapply()`는 window-only smoke helper이며 public in-game UX나 per-frame callback API가 아니다.

`authoredBehaviorVariantApply=true` query가 켜지면 예제 runtime은 scene load 이후 낮은 빈도 경로에서 variant의 spawn 대상 `SceneComposition` instance를 physics body로 만들고, `runtimeEntity: "builtinShooterPlayer"` instance는 `FerrumEngine.builtInShooterPlayerHandle()`로 현재 player handle을 조회해 같은 handle map에 넣는다. 이후 variant `ids` registry를 `FerrumEngine`의 gameplay authoring facade에 넘겨 `BehaviorRecipeCommand[]`와 FSM install plan을 Rust component storage에 적용한다. browser demo 배치는 production `game.json`의 타일맵 장애물을 피하기 위해 `semantics.browserPlacement`가 선언한 anchor/target/scale로 replay fixture 좌표를 player 중심 주변에 압축 배치한다(`runtimeApply.placementAnchorReplayBody`, `placementTarget`, `placementScale`). browser runtime smoke:

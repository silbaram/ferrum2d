...
asset diagnostics가 포함된다. UI는 이 handoff를 agent가 behavior recipe 본문, prefab binding, schema
migration을 수정할 때 참고하도록 노출하지만, handoff 생성 자체가 scene-authoring 문서를 수정하지는 않는다.

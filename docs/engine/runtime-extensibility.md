...
주의: 이 문서의 primitive는 여러 장르가 재사용할 수 있는 계약이지만, 현재 기본 Rust adapter는 built-in Shooter/Breakout/Platformer scene의 capability 안에서 실행된다. `ShooterPrefabRegistry`, `nearestPlayer`, `enemy`/`bullet` prefab kind처럼 이름에 장르가 남은 API는 호환성과 starter scene 연결을 위한 표면이며, 1.0 전에는 generic scene/runtime surface와 starter-scene surface 분리를 계속 검토한다.

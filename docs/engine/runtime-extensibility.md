...
## Platformer Authoring Convergence Proof

P2의 Platformer slice는 기존 `usePlatformerGame()` API와 platformer physics controller 동작을 유지하면서, level body 조립과 movement/landing side-effect metadata를 범용 prefab/component/reaction 경로에 올리는 단계다. Platformer controller와 one-way platform 판정은 아직 scene-specific runtime rule로 남긴다.
...
| create-game 연결 | proof only | 아직 consumer template/schema로 공개하지 않고, Platformer unit tests와 runtime-extensibility 문서가 proof contract를 고정한다. |

후속 Platformer 수렴은 create-game template/schema 공개 여부와 데이터 기반 씬 조립 경로를 별도 변경으로 나눠 진행한다.

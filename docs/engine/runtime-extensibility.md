...
주의: 이 문서의 primitive는 여러 장르가 재사용할 수 있는 계약이지만, 현재 기본 Rust adapter는 built-in Shooter/Breakout/Platformer scene의 capability 안에서 실행된다. `primaryActor` / `nearestPrimaryActor`가 장르 중립 target 이름이며, `player` / `nearestPlayer`는 호환성을 위한 legacy alias다. prefab registry/registration 계층은 `GameplayPrefabRegistry` 이름을 사용하지만, `enemy`/`bullet` prefab kind 값은 아직 starter scene 연결 표면이다. 1.0 전에는 generic scene/runtime surface와 starter-scene surface 분리를 계속 검토한다.
...
Breakout template 승격은 실제 package consumer smoke tarball 실행까지 검증했다. `pnpm package:consumer-smoke -- --artifact-dir artifacts/consumer-smoke`는 `minimal`/`topdown`/`platformer`/`breakout` 4-template matrix에서 생성, agents 설치, public import/type smoke, authoring/replay/runtime replay, production build를 확인하고, `pnpm validate:consumer-smoke-report`가 report artifact 계약을 검증한다. 후속 신규 템플릿은 같은 tarball consumer smoke matrix에 추가한다.

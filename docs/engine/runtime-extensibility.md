...
주의: 이 문서의 primitive는 여러 장르가 재사용할 수 있는 계약이지만, 현재 기본 Rust adapter는 built-in Shooter/Breakout/Platformer scene의 capability 안에서 실행된다. `primaryActor` / `nearestPrimaryActor`가 장르 중립 target 이름이며, `player` / `nearestPlayer`는 1.x 동안 입력 호환을 유지하는 legacy alias다. 신규 예제, 문서, agent-generated behavior authoring은 `primaryActor` / `nearestPrimaryActor`를 canonical target으로 사용한다. alias 제거, deprecation warning, runtime diagnostic 변경은 2.0 또는 별도 migration task에서만 다룬다. prefab registry/registration 계층은 `GameplayPrefabRegistry` 이름을 사용하지만, `enemy`/`bullet` prefab kind 값은 아직 starter scene 연결 표면이다.
...
Projectile spawn은 기존 bullet-specific storage에서 prefab/component 기반 path로 이전됐다. built-in compatibility는 유지하지만, runtime 내부는 prefab id와 component bucket을 기준으로 처리한다. public naming에는 아직 Shooter/Enemy/Bullet 호환 용어가 남아 있으므로, 이 기능은 "generic component primitive + starter scene adapter"로 본다.
...
새 movement authoring은 `primaryActor` / `nearestPrimaryActor`를 기본 target 이름으로 사용한다.
`target`을 생략한 `chase`와 `seekTarget`도 `primaryActor`로 정규화된다. 기존 `player` /
`nearestPlayer` 입력은 starter-scene 호환을 위해 계속 받지만 신규 예제와 agent-generated data의
canonical 표면으로 쓰지 않는다.
이 release policy는 movement target 이름에만 적용된다. Shooter Game Spec의 `player` key,
`nearestLayer:player`, `nearestFaction:player`, texture id `player` 같은 starter-scene 또는 layer/faction
표면은 1.x 호환 계약으로 유지한다.

지원하는 target query preset은 다음과 같다.

| Preset | 설명 |
| --- | --- |
| `primaryActor`, `nearestPrimaryActor` | `World`의 engine-owned primary actor marker를 향하는 장르 중립 target |
| `player`, `nearestPlayer` | `primaryActor` / `nearestPrimaryActor`의 legacy alias |
| `nearestEnemy` | built-in shooter compatibility target |
...
## Breakout Starter Convergence

Breakout slice는 built-in Breakout scene의 public `useBreakoutGame()` API와 render/audio/event buffer layout을 유지하면서 Rust core 내부 조립 경로를 prefab/component/reaction primitive로 옮기는 방식으로 진행한다. 현재는 create-game `breakout` template로 공개된 starter scene이며, frame hot path에 entity별 JS/Wasm callback을 추가하지 않는다.
...
Platformer slice는 기존 `usePlatformerGame()` API와 platformer physics controller 동작을 유지하면서, level body 조립과 movement/landing side-effect metadata를 범용 prefab/component/reaction 경로에 올리는 단계다. Platformer controller와 one-way platform 판정은 아직 scene-specific runtime rule로 남긴다.

현재 starter 수렴 범위:

| 항목 | 상태 | 계약 |
| --- | --- | --- |
| prefab/component spawn | 완료 | floor, wall, static platform, one-way platform, moving platform, player body는 `PrefabEntitySpawnRequest`/`EntityTemplate` 기반으로 생성한다. Platformer solid body contract 유지를 위해 collider는 `is_trigger=false`로 명시한다. |
| player movement metadata | 완료 | player controller tuning은 `MovementPattern::PlatformerInput` component에 저장하고 Platformer runtime이 controller config source로 읽는다. 기존 `TopdownInput` speed는 fallback 호환 경로로 유지한다. |
| controller tuning descriptor | 완료 | `PlatformerControllerTuning` descriptor가 horizontal speed, gravity, jump, max fall, ground probe, step, coyote, jump buffer 값을 조립한다. invalid authored value는 field 단위로 scene fallback을 사용한다. |
| moving platform path metadata | 완료 | moving platform path는 scene-local `MovingPlatformPath` descriptor로 샘플링하고, origin/amplitude/phase speed는 `MovementPattern::Oscillate` component에서 읽는다. platform carry rule은 scene rule로 유지한다. |
| landing dust side effect | 완료 | player에 authored `CollisionReaction::SpawnParticle`를 설치하고 landing state transition에서 shared tile side-effect helper로 cooldown/target transform을 평가한다. landing trigger 자체는 platformer controller 결과를 사용한다. |
| platformer controller | scene-specific | controller 실행과 one-way platform rule은 기존 `PhysicsSystem::move_platformer_controller_with_state_and_counters(...)` 경로를 유지한다. config 값은 `PlatformerControllerTuning` metadata source에서 조립한다. |
| create-game 연결 | starter scaffold + authoring/replay fixtures | `packages/create-game/templates/platformer`가 public `usePlatformerGame()` starter, scene authoring fixture, gameplay replay template-surface fixture, headless runtime replay fixture를 제공한다. Platformer 전용 built-in snapshot은 아직 없으므로 runtime replay는 `custom.platformer` JSON으로 canonical state를 고정한다. |

후속 Platformer 수렴은 공개 starter template을 유지하면서 데이터 기반 씬 조립 범위 확대와 Platformer 전용 snapshot/API 승격 여부를 별도 변경으로 나눠 진행한다.
...
- TypeScript authoring은 projectile/weapon, `SceneComposition`, Behavior Recipe, presentation effect 정의를 serializable data와 numeric command/apply plan으로 compile한다.
...
- 세션 단위 `FactionRelationTable`은 TS relation spec, Rust relation table storage, existing `GameplayFaction.damage_mask` legacy fallback, authoritative table damage gate, snapshot/replay hash migration을 포함한다. faction/tag id는 현재 `u32` mask-backed `0..31` ABI 계약으로 유지하며, `32` 이상 확장은 snapshot/replay migration을 포함하는 별도 설계 단위다.

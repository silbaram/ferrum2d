...
범용성 핵심 구현은 완료됐다. 남은 항목은 필수 runtime primitive가 아니라 제품화 polish다.

- starter-runtime에서 homing missile/explosive projectile demo profile을 더 직접 보여주기
- query result debug buffer로 nearest target 선택 결과를 bulk telemetry에 노출
- planning 문서의 완료 기록을 public API/architecture/smoke 기준 문서로 계속 이관

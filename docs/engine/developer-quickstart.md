...
| 어디까지 지원하나? | WebGL2/WebGPU rendering, input, audio, asset loading, tilemap, physics, built-in starter scenes |
...
| `platformer` | built-in platformer scene | jump, movement, platform collision, side-scroller starter |
| `breakout` | built-in Breakout scene | paddle, ball, brick/session 흐름의 arcade starter |

...
현재 예제 설정의 기준은 실제 `game.json`이고, 문서용 필드 설명의 기준은 [Top-down Shooter Game Spec](topdown-shooter-game-spec.md)이다. 실제 예제는 runner/bruiser/orbiter wave와 10x6 tilemap obstacle layer를 포함한다. 아래는 구조를 빠르게 보기 위한 축약 예시다.
...
필드별 타입, 기본값, 검증 조건은 [Top-down Shooter Game Spec](topdown-shooter-game-spec.md)의 `필드` 표를 확인한다.
...
| `docs/engine/topdown-shooter-game-spec.md` | Top-down Shooter 예제 설정 필드의 상세 설명 |

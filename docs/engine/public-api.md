...
| `createRenderer(...)` | 기본 WebGL2 renderer를 생성한다. `preferred: "webgpu"`는 optional renderer를 먼저 시도한 뒤 실패 시 WebGL2로 fallback한다. |
...
| `summarizeScreenshotPixels(...)`, `compareScreenshotSummaries(...)` | browser smoke/readback screenshot summary와 baseline threshold 비교 report를 만드는 품질 인프라 helper다. |
| `resolveShooterGameSpec(...)` | Top-down Shooter starter scene용 Game Spec 기본값과 검증을 적용한다. |
| `applyShooterGameSpec(...)` | 검증된 Top-down Shooter starter scene Game Spec을 Rust engine에 적용한다. |
...
| `createPixelMaskTerrain(...)`, `extractPixelMaskBoundaryChains(...)` | alpha mask 기반 terrain helper와 Physics Spec chain boundary 변환을 제공한다. Destructible terrain gameplay loop는 template/adapter가 소유한다. |
| `createPixelMaskTerrainRuntime(...)` | pixel mask terrain의 texture upload와 chunk collider rebuild/ownership을 낮은 빈도 runtime adapter로 조율한다. |
...
| `createPhysicsReplayWorkerClient(...)` | Physics Spec snapshot/replay 검증을 Web Worker에서 실행하고 transfer benchmark를 측정하는 opt-in client를 만든다. 전체 게임 루프 Worker 이전 기능은 아니다. |
| `compilePhysicsAuthoringDocument(...)` | `physicsEditor` metadata를 제거하고 runtime `PhysicsSpec`만 export한다. |
| `diagnosticReport(...)` | runtime/package 진단 정보를 만드는 품질 인프라 helper다. |
...
| Core scene | `score`, `entityCount`, `gameState`, `resetGame`, `useDataScene`, `setViewportSize` |
| Starter scene | `useBreakoutGame`, `usePlatformerGame`, `setGameSpec`, `builtInShooterPlayerHandle`, built-in shooter snapshot, shooter tilemap edit helpers |
...
| `FerrumRuntimeHud`, `FerrumRuntimeHudOptions` | `hudToolkit` component preset을 runtime `UiOverlay` panel로 연결한다. |
| `FerrumRuntimeAccessibility`, `FerrumRuntimeAccessibilityOptions` | accessibility option을 resolved handle로 노출하고 subtitle panel/theme hook을 runtime `UiOverlay`에 연결한다. |
| `FerrumRuntimeAnimationTimeline`, `FerrumRuntimeAnimationTimelineOptions` | `AnimationTimelinePlayer`를 frame-end에 갱신하고 snapshot/signal/pause/resume/onUpdate hook을 제공한다. |
...
| `FerrumRuntimeLocalization` | `LocalizationBundle`을 runtime handle로 노출하고 locale 변경과 `t(...)` 호출을 제공한다. |
| `FerrumRuntimeCutscene` | `CutsceneSequencePlayer`를 frame-end에 갱신하고 snapshot/reset/skip/pause/resume/UI state를 제공한다. |
...
`FerrumRuntimeOptions.engine`은 runtime이 새 `FerrumEngine`을 만들 때 전달하는 `CreateEngineOptions`다. 테스트 하네스나 커스텀 bootstrap이 이미 만든 엔진을 재사용해야 하면 `FerrumRuntimeOptions.engineInstance`에 기존 `FerrumEngine`을 전달할 수 있다. 이 경우 runtime은 `start`/`pause`/`resume`/`stop` lifecycle forwarding은 유지하지만 `destroy()`에서 주입 엔진을 파괴하지 않는다.
...
`FerrumRuntimeOptions.hud`에 static component 배열 또는 runtime frame provider를 전달하면 `runtime.hud` handle과 HUD panel `UiOverlayState`가 생성된다. `FerrumRuntimeOptions.accessibility`는 `resolveAccessibilityOptions(...)` 결과를 `runtime.accessibility` handle로 노출하고, subtitle provider가 있으면 subtitle status panel을 만든다. `ui.theme`을 직접 지정하지 않은 경우 accessibility contrast palette의 HUD theme이 `UiOverlay` 기본 theme으로 적용되며, `applyUiTheme: false`로 끌 수 있다. `FerrumRuntimeOptions.animationTimeline`은 spec 또는 `AnimationTimelinePlayer`를 frame-end에 갱신하고 frame event/state transition 결과를 `onUpdate`로 노출한다.

`FerrumRuntimeOptions.localization`에 localization document 또는 `LocalizationBundle`을 전달하면 `runtime.localization` handle이 생성된다. `FerrumRuntimeOptions.cutscene`은 `CutsceneSequencePlayer`를 프레임 끝에서 낮은 빈도로 갱신하며, dialogue command는 기본 `UiOverlay` dialog state로 표시할 수 있다. `cutscene.dialogue.textMode: "localizationKey"`를 쓰면 command `text`를 runtime localization key로 해석한다. audio command는 BGM이면 `AssetHost.playBgm(...)`/`stopBgm(...)` optional hook으로 `loop`/`fadeSeconds`/`stop`을 보존하고, custom host가 해당 hook을 제공하지 않으면 기존 `AssetHost.playAudioEvents(...)` fallback을 사용한다. 카메라 이동과 장르별 narrative side effect는 `CutsceneSequenceTarget` adapter가 담당한다.
...
`GameStateSnapshot`은 세이브 파일용 versioned JSON envelope다. 현재 범위는 `FerrumEngine`에서 조회 가능한 scene metric(`score`, `gameState`, entity/sprite count, camera), 선택적 data scene state, 선택적 built-in shooter state, 선택적 `PhysicsWorldSnapshot`, 게임별 `custom` JSON state를 하나로 묶는 것이다.

| 타입/API | 역할 |
| --- | --- |
| `captureGameStateSnapshot(...)` | runtime scene metric을 캡처하고, `includeDataSceneState`가 `true`이면 `DataSceneStateSnapshot`을, `includeBuiltInShooterState`가 `true`이면 built-in shooter state를, `physicsWorld`가 전달되면 `capturePhysicsWorldSnapshot(...)` 결과를 포함한다. data scene과 built-in shooter state는 같은 snapshot에 함께 넣지 않는다. |
| `restoreGameStateSnapshot(...)` | snapshot의 data scene state가 있으면 `FerrumEngine.useDataScene()`으로 data scene mode를 복원하고, built-in shooter state와 PhysicsWorld를 복원한다. `applyDataSceneCustomState`/`applyCustomState` callback이 있으면 JSON custom state를 적용한다. |
| `hashGameStateSnapshot(...)` | scene/dataScene/physics/custom 내용을 canonical JSON hash로 계산한다. |
...
| `FerrumEngine.useDataScene()` | built-in Shooter/Breakout/Platformer scene runtime을 실행하지 않는 빈 data scene mode로 전환한다. 전환 직후 score는 `0`, game state는 playing, entity/render/audio output은 비워지며 이후 generic `World` update와 tilemap dynamic collision만 Rust frame loop에서 실행된다. |
| `DataSceneStateSnapshot` | `format: "ferrum2d.data-scene-state"`, `version: 1`, 현재 `scene` metric, optional JSON `custom`을 담는 data scene 전용 save/replay payload다. 새 Rust ptr/len buffer ABI를 만들지 않으며, built-in Shooter snapshot layout과 분리된다. |
| `BuiltInShooterStateSnapshot`, `captureShooterStateSnapshot()`, `restoreShooterStateSnapshot(...)` | built-in shooter의 score, game state, spawn/wave timer, camera, player/enemy/bullet runtime state, 세션 faction relation table, player primary projectile/dash/melee/spawnPrefab action binding/cooldown을 낮은 빈도 save/load용 buffer로 캡처하고 복원한다. |
...
주의: data scene restore는 현재 data scene mode 전환과 JSON custom callback 적용을 대상으로 한다. generic `World` 전체 component restore를 약속하지 않으며, 필요한 entity/component 복원은 별도 versioned payload가 생기기 전까지 게임별 custom state나 PhysicsWorld snapshot 같은 명시적 경로로 관리한다. built-in shooter restore는 gameplay state를 대상으로 한다. 점수, game state, spawn/wave timer, camera, player/enemy/bullet position/velocity/health/damage/lifetime/reward와 in-flight bullet collision target/tile impact/source faction, 세션 faction relation table, player primary projectile/dash/melee/spawnPrefab action binding/cooldown은 복원하지만, hit flash, particle, tween, pending spawn/melee command, pending audio event 같은 순간 효과는 세이브 파일에 포함하지 않는다.

`BuiltInShooterStateSnapshot` version `17`은 player primary action의 projectile tile impact policy, in-flight bullet collision target/tile impact/source faction policy, dash/melee action binding/cooldown slot, player spawnPrefab action binding/cooldown 반복 슬롯, registered Enemy/Bullet prefab alias id, spawnPrefab projectile payload, Rust input action registry, full previous `InputState` edge snapshot, prefab registry canonical payload, faction relation table `enabled/default + friendly/hostile bitplane` payload를 포함한다. version `17`은 header float `8`, header u32 `151`, entity float stride `131`, entity u32 stride `117`을 public validation에서 고정한다. version `17`은 version `16`에서 player spawnPrefab action binding 반복 슬롯을 16개로 확장한 format bump다. version `16`은 version `15`에서 세션 단위 faction relation table을 추가한 format이었다. spawnPrefab 반복 슬롯은 현재 `ActionBindingSet` capacity에 맞춰 최대 16개를 action id 오름차순 canonical order로 저장하고, restore도 gap이나 내림차순 slot을 거부해 recapture hash drift를 막는다. snapshot `version`과 `headerU32s[0]`는 같은 version code여야 하며, restore는 중복 action id, capacity 초과 action binding snapshot, invalid faction relation table snapshot, 이전 snapshot version을 거부한다. `restoreGameStateSnapshot(..., { restoreBuiltInShooterState: false })`로 built-in shooter state 적용을 끌 수 있다.

Gameplay replay helper는 `GameStateSnapshot`을 frame 단위 canonical gameplay snapshot으로 사용한다. golden 비교 대상은 scene metric, optional data scene state, built-in shooter state, optional PhysicsWorld/custom JSON이며 render command, audio event, wall-clock profiler, debug overlay는 snapshot/hash 범위에 들어가지 않는다. 이 API는 runtime loop를 대신 실행하지 않는다. smoke나 예제 harness가 fixed timestep/seed/input sequence로 프레임을 실행하면서 필요한 frame에서 `captureGameStateSnapshot(..., { includeBuiltInShooterState: true })` 또는 data scene용 `captureGameStateSnapshot(..., { includeDataSceneState: true })`를 호출하고, 그 결과를 `createGameplayReplayRun(...)`에 넘기는 구조다.
...
- `WebGL2Renderer.evictTexture(textureId)`, `WebGPURenderer.evictTexture(textureId)`: 청크 언로드 같은 낮은 빈도 경계에서 renderer texture registry의 id 기반 texture를 해제한다. placeholder id `0`은 유지된다.
...
| `createFerrumRuntime({ animationTimeline })` | spec 또는 player를 runtime frame-end에 연결하고 `runtime.animationTimeline` handle과 `onUpdate` hook을 제공한다. |
...
await createFerrumRuntime({
  ...
  animationTimeline: {
    timeline,
    signals: ({ frame }) => frame.gameState === 1 ? ["move"] : [],
    onUpdate: (result) => {
      for (const event of result.events) {
        // frame-end adapter가 event metadata를 읽는다.
      }
    },
  },
});
...
Scene composition은 AI agent가 prefab, variant, scene fragment를 데이터로 조립하기 위한 저빈도 authoring helper다. 엔진 core나 render command ABI를 바꾸지 않고, 게임별 runtime adapter가 flat instance 목록을 받아 entity/physics/UI spawn으로 변환하는 구조다. `SceneComposition`과 `BehaviorRecipe`는 현재 `ShooterGameSpec` top-level namespace로 승격하지 않고, consumer template과 agent workflow가 읽는 별도 `scene-authoring` artifact로 유지한다.
...
| `SCENE_AUTHORING_DOCUMENT_FORMAT`, `SCENE_AUTHORING_DOCUMENT_VERSION` | consumer `public/scene-authoring.json` envelope의 canonical format/version |
| `resolveSceneAuthoringDocument(...)` | `format`, `version`, `sceneComposition`, `behaviorRecipes`, optional `ids`를 검증하고, 요청 시 scene instance와 behavior binding plan도 non-mutating으로 검증한다. |
| `applySceneBehaviorRecipes(...)` | scene instance를 spawn target으로 적용해 entity handle map을 만들고, binding된 `BehaviorRecipeCommand[]`를 기존 gameplay authoring facade로 적용한다. |
...
| `applySceneBehaviorRecipes(...)` | `SceneBehaviorRuntimeTarget.spawnSceneInstance(...)`가 반환한 `GameplayEntityHandle`로 handle map을 만들고 `applyGameplayBehaviorCommands(...)`를 실행한다. |
...
| `applyFactionRelationTable(...)` | 세션 단위 directed faction relation table을 낮은 빈도로 적용한다. 기본 relation과 source/target pair별 `neutral|friendly|hostile` 값을 검증하고 Rust relation table setter로 위임한다. |
...
| `engine.applyFactionRelationTable(...)` | `applyFactionRelationTable(engine, ...)`와 같은 검증/적용 경로를 `FerrumEngine` method로 실행한다. |
...
`runBehaviorStateMachineReplay(...)`는 production runtime이 아니라 agent validation용 transition dry-run이다. input은 `machine`, 대상 `entity` handle, strictly increasing `frames`를 요구한다. raw FSM document를 replay할 때는 `behaviorRecipes` option을 요구해 profile reference를 검증한다. replay는 Rust runtime과 동일하게 대부분의 frame event에서 `source` entity id/generation이 input `entity`와 일치하는 event만 transition 후보로 보며, `pickupCollected`만 collector `actor`를 subject로 삼는다. `interaction` replay predicate는 canonical `actionId` 또는 `ids.actions[action]`으로 해석 가능한 action name을 가져야 하며, `action` 문자열은 사람이 읽는 metadata와 registry key로 유지한다. `timer` replay predicate는 canonical `timerId` 또는 `ids.timers[timer]`으로 해석 가능한 timer name을 가져야 한다. `pickupCollected` replay predicate는 canonical `itemId` 또는 `ids.items[item]`으로 해석 가능한 item name을 가져야 한다. `animationFrame` replay predicate는 canonical `animationTokenId` 또는 `ids.animationEvents[animationToken]`으로 해석 가능한 animation event name을 가져야 한다. `tileImpact` replay predicate는 emitted tile impact policy/code `0(despawn)` 또는 `2(bounce)`를 canonical key로 삼는다. unknown tile impact telemetry는 replay input으로 관측될 수 있지만 어떤 predicate와도 match되지 않는다. `collisionDamage`/`collisionDespawn` replay predicate는 event kind 자체를 canonical key로 삼고 action id를 받지 않는다. transition 평가는 state의 transition 배열 순서를 우선하며 frame당 최대 1개 transition만 발생한다. replay hash에는 대상 entity handle, matched event의 actor/source entity id와 generation, collision damage/targetRemoved, timer id/duration, pickup item/count, animation token/clip/frame, tile impact policy/code와 tile identity/normal/flags metadata가 포함되어 stale handle과 event outcome 변화도 diff에 잡힌다.

`createBehaviorStateMachineRuntimeInstallPlan(...)`은 state id를 문자열 코드 단위 정렬 순서의 positive integer로 고정하고, runtime transition에는 event kind code와 token id를 넣는다. interaction token id는 positive `actionId` 또는 `ids.actions[action]`, timer token id는 positive `timerId` 또는 `ids.timers[timer]`, pickupCollected token id는 positive `itemId` 또는 `ids.items[item]`, animationFrame token id는 positive `animationTokenId` 또는 `ids.animationEvents[animationToken]`, tileImpact token id는 `0(despawn)` 또는 `2(bounce)`이며, collisionDamage/collisionDespawn token id는 `0`이다. 생성된 numeric state id는 install plan 내부 계약이다. save data나 authoring spec에는 생성된 숫자가 아니라 state 문자열을 유지해야 하며, state 추가/이름 변경 시 numeric id는 달라질 수 있다. 설치 helper는 scene load 또는 agent apply 같은 낮은 빈도 경로에서만 쓰며, 먼저 기존 FSM component를 clear한 뒤 initial state와 bounded transition set을 Rust setter로 넣는다. transition 적용 중 실패하면 다시 clear해 partial FSM component를 남기지 않는다. 이전 FSM을 복원하는 transaction API는 아니다.
...
| `createFerrumRuntime({ hud })` | static component 배열 또는 frame provider를 HUD panel로 변환해 runtime `UiOverlay`에 병합한다. |
...
| `createFerrumRuntime({ localization })` | localization document 또는 bundle을 runtime handle로 연결해 cutscene/dialogue/HUD adapter가 같은 locale table을 공유할 수 있게 한다. |
...
| `createFerrumRuntime({ dialogue })` | opt-in dialogue session을 runtime `UiOverlay`와 action handler에 연결한다. 기존 `uiState`가 dialog를 반환하면 앱 dialog를 우선하고, 그렇지 않으면 현재 dialogue node를 표시한다. |
  ...
  dialogue: {
    graph: {
      ...
      nodes: {
        welcome: {
          speaker: "Guide",
          text: "Welcome to Ferrum2D.",
          choices: [{ id: "next", label: "Continue", to: "done" }],
        },
        done: { speaker: "Guide", text: "Ready.", end: true },
      },
    },
    ui: { dialogId: "guide", title: "Guide" },
  },
});
```

`dialogue`와 `ui: false`를 함께 지정하면 `runtime.dialogue` 핸들은 생성되지만 `UiOverlay`가 비활성화되므로 대화는 화면에 자동 표시되지 않는다. 이 조합은 게임이 자체 UI로 `runtime.dialogue.uiState()`를 소비할 때 사용한다.
...
- `ShooterTilemapSpec`, `ShooterTileLayerSpec`, `ShooterTileLayerChunkSpec`, `ShooterTileSpec`, `ShooterTileKind`, `ShooterTileRampSpec`, `ShooterTileSlopeSpec`, `ShooterTileBridgePortalSpec`: tilemap, chunked layer input, one-way, slope, bridge portal, HD-2D tile metadata
- `ShooterAtlasAnimationSpec`, `ShooterAtlasAnimationStateSpec`, `ShooterAtlasAnimationFrameEventSpec`: atlas clip, non-looping attack/die clip, frame event metadata
...
`BrowserPlatformHost`는 `AssetHost.playBgm(...)`와 `AssetHost.stopBgm(...)` optional hook을 제공한다. Custom `AssetHost`는 기존 `playAudioEvents(...)`만 구현해도 호환되지만, cutscene BGM의 `loop`, `fadeSeconds`, `stop` 의미를 보존하려면 두 BGM hook을 함께 구현해야 한다.
...
| `createFerrumRuntime({ cutscene })` | opt-in cutscene player를 frame-end에 갱신하고 dialogue command를 runtime `UiOverlay` state에 연결한다. BGM audio command는 `AssetHost.playBgm(...)`/`stopBgm(...)`가 있으면 `loop`/`fadeSeconds`/`stop`을 보존하고, 없으면 기존 `playAudioEvents(...)` fallback을 사용한다. |
...
await createFerrumRuntime({
  ...
  cutscene: {
    sequence: {
      id: "intro",
      commands: [
        { kind: "dialogue", speaker: "Guide", text: "intro.ready", durationSeconds: 2 },
      ],
    },
    dialogue: {
      title: "Intro",
...
| `createFerrumRuntime({ accessibility })` | resolved accessibility option을 runtime handle로 노출하고 subtitle panel 및 contrast HUD theme을 `UiOverlay`에 연결한다. |

...
공식 `examples/**`와 `packages/create-game/templates/**`는 새 consumer 코드의 기준이므로
compatibility root aggregate import를 사용하지 않는다. 이 경로들은
`docs/engine/public-api-surface.json`의 `rootAggregateImportRestrictedRoots`로 관리하며,
`pnpm validate:public-api-surface`가 목적별 subpath 사용 여부를 검사한다.
...
| [Authoring](public-api/authoring.md) | scene composition, Data Scene spawn/role/handle registry, Scene Placement Authoring Viewer state/viewport/patch merge/save helper, behavior recipe, gameplay command, FSM, physics authoring facade |

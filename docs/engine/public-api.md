...
| API | 용도 |
| --- | --- |
| `AssetLoader` / `AudioAssetLoader` | texture/sound/JSON asset loading. `AssetLoader.releaseAssets(...)`는 현재 registry의 `(name, url)`과 release payload가 일치하는 texture/sound만 platform manager eviction으로 위임한다. |
...
주의: level streaming runtime adapter는 Rust simulation state를 직접 바꾸지 않는다. Chunk 단위 Game Spec apply, pixel mask terrain sync, Physics Spec collider rebuild는 target callback에서 낮은 빈도 작업으로 수행해야 한다. `FerrumRuntimeLevelStreamingUpdateResult.releasedAssets`와 `target.releaseAssets`는 unload된 chunk asset 중 현재 preload/retain window에서 같은 `(kind, name, url)`로 더 이상 참조하지 않는 entry를 `entries`와 kind별 배열로 제공한다. `createFerrumRuntime({ assetHost, levelStreaming })`는 release payload를 user target callback에 전달한 뒤 `AssetHost.releaseAssets(...)`에도 전달한다. 기본 `BrowserPlatformHost`는 해당 payload를 `AssetLoader.releaseAssets(...)`로 위임해 현재 registry의 URL과 일치하는 WebGL texture와 decoded sound buffer만 해제하고, JSON entry는 preload/cache invalidation과 분리된 no-op으로 둔다. `preload.cache`와 `preload.cachePolicy`에 `IndexedDbAssetCache`를 전달하면 기존 asset preload cache layer를 그대로 사용한다.

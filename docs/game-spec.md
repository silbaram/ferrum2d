...
| `tilemap.layers.*.collision` | boolean | `false` | `true`이면 양수 tile id를 player/enemy가 통과할 수 없는 정적 AABB 장애물로 사용 |
...
`tilemap.tiles`의 key는 positive integer string이어야 한다. `0`은 빈 타일로 예약되어 layer data에서만 사용할 수 있다. `collision: true` layer의 양수 tile은 player/enemy 이동을 막는 정적 AABB로 해석되며 bullet 충돌, 자동 타일링, editor, per-tile script, pathfinding은 포함하지 않는다.

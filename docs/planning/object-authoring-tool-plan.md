...
- Tauri wrapper: `apps/placement-viewer-desktop`에서 window open, 기본 샘플/환경변수/직접 scene 문서 경로/project root 직접 경로/native file dialog/project folder 기반 scene-authoring JSON load, asset folder picker/inspect, Save action local write, `.ferrum-placement-handoff.json` 자동 sync/수동 저장, Inspector project/assets/source/handoff/save 상태 표시까지 spike 완료. 다음은 동적 runtime texture loading과 packaging 검증 판단이다.

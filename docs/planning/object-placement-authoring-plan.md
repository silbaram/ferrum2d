...
1. Slice 2(`instance.id`↔`Entity` 매핑, explicit id/stale handle/reorder 안정성)를 이어서 진행한다.
2. Slice 0/1A/1B가 완료 상태로 유지되도록 새 schema/API 필드는 `docs/engine/data-scene-authoring.md`, public API surface, sample validator를 함께 갱신한다.
3. 배치 오버레이(Slice 4)는 승인 후 `examples/*` dev 모드 프로토타입으로 시작한다.

...
- **완료**: Slice 1~6의 Data Scene apply, instance handle registry, placement editing/patch, agent behavior binding, official/generated viewer, `World` 장르 누수 정리를 완료했다.
- **운영 계약**: authoring schema와 사용법은 [Data Scene Authoring Contract](../engine/data-scene-authoring.md), public API, placement viewer README가 기준이다. 이 문서의 완료 로그는 결정 근거다.
- **활성 후보**: fragment hierarchy v2 UX, scene-specific compatibility API, desktop packaging/GUI release 범위를 결정한다. local image dimension metadata는 완료됐다.
- **별도 승인**: full visual editor, node/prefab body editor, behavior 본문/FSM/action graph 편집은 이 계획에 포함하지 않는다.
...
- 기존에 가장 큰 끊어진 다리였던 **`SceneComposition` instance → 실행되는 `World` 엔티티** 경로는 Slice 1A/1B에서 해결했다. `props.components` v1 schema, Rust/Wasm raw spawn hook, package-facing `applyDataSceneAuthoringDocument(...)`/`createDataSceneRuntimeTarget(...)`, `classifySceneInstance(...)`, 샘플 spawn smoke, visible sprite rotation/render layer 연결이 현재 기준이다.
...
- **정식 Scene Placement Authoring Viewer**, **agent 기능 부착 루프 검증**, **`World` 장르 누수 정리**는 완료했다. `player`/`nearestPlayer`는 1.x 입력 호환 alias로 유지하고 신규 authoring 표면은 `primaryActor`/`nearestPrimaryActor`를 canonical로 사용한다.
- 남은 범위는 기존 기반의 재구현이 아니라 fragment hierarchy, scene compatibility, desktop release 범위에 대한 제품 결정이다.
...
- **명시 id 운영 정책.** UI/agent apply 경로는 `requireExplicitInstanceIds: true`와
  `addInstance(...)`의 explicit `instance.id` 검증을 사용한다. Core viewer API는 id를 자동
  생성하지 않고 invalid/duplicate id를 거절한다. 공식 `apps/placement-viewer`는 add id 입력이
  비어 있으면 primitive/prefab/sprite prefix 기반 deterministic id를 만든 뒤 `addInstance(...)`를
  호출하고, generated create-game viewer는 `sprite-*`/`object-*` prefix helper로 같은 정책을 따른다.
  `renameInstance(...)`/`removeInstance(...)`는 agent-owned behavior 본문을 직접 rewrite하지 않고
  `previewScenePlacementBindingMigration(...)` reference report로 저장 차단 또는 handoff evidence를
  남긴다.
- **primary actor 호환 경로.** 범용 `World`의 내부 단일 플레이어 저장소는 `primary_actor`로 중립화했고, engine-owned reserved `GameplayTags` marker와 동기화한다. movement authoring은 `primaryActor`/`nearestPrimaryActor` generic target을 canonical 표면으로 쓰며 `chase`/`seekTarget` 생략 target도 `primaryActor`로 정규화한다. `player`/`nearestPlayer`는 1.x 입력 호환 alias로 유지하고 신규 authoring 표면에는 쓰지 않는다. prefab registry/registration 계층은 `GameplayPrefabRegistry`로 중립화했고, `World`의 `player_entity()` 호환 accessor는 제거했다.
- **built-in scene 이중 구조.** Shooter/Breakout/Platformer는 하드코딩 Rust scene이고, 제네릭 Data Scene(`useDataScene()`)은 별도 generic ECS tick이다(`engine/scenes.rs`). built-in scene의 공통 reset/update/camera/telemetry hook은 `BuiltInSceneRuntime` trait와 `BuiltInSceneSlots` active runtime accessor로 수렴했고, concrete scene 필드는 slots 내부 private state로 격리했다. 배치 UI + agent 흐름은 본질적으로 **Data Scene** 경로다.
...
- **Scene 추상화 승격**: 사용자 승인 하에 1차 구현을 시작했다. 현재 완료 범위는 built-in scene concrete 필드 캡슐화, active runtime trait dispatch 수렴, `createFerrumRuntime({ dataScene })` startup apply, `GameStateSnapshot.dataScene.authoringDocument` 기반 Data Scene restore 1차 경로다. public snapshot/API를 더 넓은 scene taxonomy나 scene-specific compatibility API까지 일반화하는 변경은 별도 후속 범위로 남긴다.
...
- **Slice 3C 완료**: 정식 `apps/placement-viewer` host와 root `pnpm dev:placement-viewer` 실행 경로를 추가하고, Data Scene preview + selected id/entity handle inspector hook을 연결했다.
...
- TS package facade는 `applyDataSceneAuthoringDocument(engine, document, options?)`와 `createDataSceneRuntimeTarget(engine, options?)`로 추가했다. full document helper는 scene-authoring envelope를 검증한 뒤 Data Scene target 생성과 behavior recipe apply를 한 번에 수행한다. target helper는 resolved inline `props.components`를 raw Wasm spawn 인자로 컴파일해 `applySceneBehaviorRecipes(...)`에 넘길 수 있는 `spawnSceneInstance(instance)` target을 제공한다.
...
- `@ferrum2d/ferrum-web/authoring`에 `applyDataSceneAuthoringDocument(...)`를 제공해 scene-authoring 문서 하나로 Data Scene runtime assembly를 수행하고, `createDataSceneRuntimeTarget(...)`를 제공해 낮은 수준의 `SceneBehaviorRuntimeTarget`도 유지한다.
...
- 샘플 scene을 `createEngine(...)` + `applyDataSceneAuthoringDocument(...)`로 spawn하는 ferrum-web test
...
- rename은 단순 string replace가 아니라 mapping migration이다. UI는
  `sceneComposition.fragments[].instances[].id`만 바꾸고 agent-owned behavior profile 본문은 보존한다.
  필요한 binding reference는 `previewScenePlacementBindingMigration(...)` report로 분리해 agent patch나
  handoff evidence에서 처리한다.
...
- explicit id validation과 UI 저장기 id 부여 정책(완료)
...
- `apps/placement-viewer`에서 data scene preview 실행(완료)
...
- 새 instance 생성 시 explicit id 자동 부여와 중복 id 방지(완료, official/generated viewer auto naming과 add API duplicate validation)
...
`World.player` 내부 저장소와 prefab spawn marker는 `primary_actor` / `primary_actor_marker`로 정리했고, primary actor 지정/해제는 reserved `GameplayTags` marker를 동기화한다. `MovementTarget`과 raw movement setter에는 `primaryActor` / `nearestPrimaryActor` generic path를 추가했고, TS behavior recipe의 `chase`/`seekTarget` 기본 target도 `primaryActor`로 정규화했다. 기존 `player` / `nearestPlayer`는 1.x 입력 호환 alias로 유지하되 신규 문서/예제/agent-generated behavior data의 canonical target으로 쓰지 않는다. Shooter Game Spec의 `player` key, layer/faction `player`, texture id `player`는 starter-scene 표면으로 별도 유지한다. alias 제거, deprecation warning, runtime diagnostic 변경은 2.0 또는 별도 migration task에서만 다룬다. prefab registry/registration/resolved-components 계층은 `GameplayPrefabRegistry` / `GameplayPrefabRegistration` / `GameplayPrefabResolvedComponents`로 중립화했고 snapshot layout은 유지한다. 기존 shooter/platformer 호환 accessor인 `player_entity()`는 제거했고, built-in scene 내부는 `primary_actor_entity()`를 호출한 뒤 로컬 변수명으로만 player 개념을 유지한다.
...
- `props.components` v1 schema는 inline descriptor를 public authoring contract로 둔다. catalog/reference 확장은 별도 migration task에서 다룬다.
- texture authoring id 해석은 기본 `AssetHost.textureId(name)`/project asset provider resolver를 사용한다. scene-authoring 문서 내부 asset registry는 아직 도입하지 않는다.
- v1 편집 단위는 단일 visible fragment 평면 편집이다. fragment 계층 보존 편집과 절대/상대 transform 역변환은 후속 범위다.
- 프리뷰는 `useDataScene()` live spawn을 기본값으로 한다. editor-only renderer는 fallback 후보일 뿐 제품 계약이 아니다.
- prefab 정의 source는 Rust `EntityTemplate`/`PrefabEntitySpawnRequest` 경로로 낮춘다. 별도 runtime scene-graph/GameObject catalog는 도입하지 않는다.
- viewer host 위치는 engine repo의 `apps/placement-viewer`와 generated consumer project의 `npm run ferrum:placement-viewer` route/script로 확정했다.
- 저장은 `ScenePlacementPatch` export와 host-provided `saveScenePlacementPatch(...)` adapter를 모두 지원한다. save adapter는 explicit opt-in/allowlist가 필요하다.
- desktop wrapper는 Tauri spike로 진행했고 local scene document/project open, asset folder inspect, `ferrum-asset://` preview URL, runtime texture registration/reload, handoff sync를 `pnpm smoke:placement-viewer-desktop-assets`로 검증한다.

남은 선택:

- fragment hierarchy를 직접 편집하는 v2 authoring UX를 만들지 여부.
- scene-specific compatibility API를 public surface로 둘지, Data Scene + built-in scene adapters로 계속 분리할지.
- desktop wrapper packaging/GUI 수동 검증을 product release 범위에 넣을지. local image dimension metadata는 official viewer와 desktop asset smoke 범위에서 완료됐다.

## 별도 승인 필요 범위

- full visual editor 제품화: 노드 트리, 풀 prefab editor, 멀티유저 동시 편집, behavior UI 편집은 별도 승인.
- Scene 추상화 승격 중 신규 scene taxonomy/API naming, replay fixture 정책, `World.player` 계약 변경: 별도 task 또는 이슈로 설계 합의 선행.
...
1. Scene 추상화의 다음 단계로 scene-specific compatibility API를 별도 공개 표면으로 둘지 결정한다. Data Scene runtime boot/snapshot/restore 기준은 `data-scene-authoring-snapshot-restore` golden smoke fixture를 유지한다.
2. fragment hierarchy v2는 실제 authoring 요구가 확인될 때 transform 역변환과 patch 호환성 범위를 먼저 설계한다.
3. desktop packaging/GUI는 release 포함 여부를 승인받은 뒤 별도 task로 분리한다.

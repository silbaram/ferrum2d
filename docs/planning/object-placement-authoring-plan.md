...
- 기존에 가장 큰 끊어진 다리였던 **`SceneComposition` instance → 실행되는 `World` 엔티티** 경로는 Slice 1A/1B에서 해결했다. `props.components` v1 schema, Rust/Wasm raw spawn hook, package-facing `createDataSceneRuntimeTarget(...)`, 샘플 spawn smoke가 현재 기준이다.
- 이제 남은 핵심 다리는 **`instance.id` ↔ `Entity` 지속 매핑**이다. 이 매핑이 있어야 선택·프리뷰·agent 타겟팅이 scene reload/reapply 후에도 안정적으로 동작한다.
- 따라서 남은 보강 순서는 다음과 같다.
  1. **`instance.id` ↔ `Entity` 지속 매핑**: 선택·프리뷰·agent 타겟팅의 토대.
  2. **배치 프리뷰와 editor-only 저장/merge**: UI가 위치 키만 갱신하고 agent-owned behavior를 보존한다.
...
- **지속 mapping 없음.** `applySceneBehaviorRecipes(...)`는 spawn 결과를 반환하지만, scene reload/reapply 이후 `instance.id`와 live `Entity` handle을 장기 보관·검증·갱신하는 registry는 아직 없다.
...
- **Slice 1B 완료**: Rust/Wasm raw data-scene spawn hook, TS package default `spawnSceneInstance` target, public API/docs, 샘플 spawn smoke test를 완료했다.

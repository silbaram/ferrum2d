...
- `@ferrum2d/ferrum-web/authoring` public export: `createScenePlacementViewer(...)`, viewer state/option 타입
- `PlacementViewport` helper: canvas CSS pixel, backbuffer pixel, DPR, camera, zoom, pan, world bounds 변환
- `screenToWorld(...)`, `worldToScreen(...)`, `snapWorldPoint(...)` 단위 테스트
- `examples/placement-viewer` 또는 create-game template viewer route에서 data scene preview 실행
- repo root 실행 명령 후보: `pnpm dev:placement-viewer`
- generated consumer project 실행 명령 후보: `npm run ferrum:placement-viewer` 또는 `pnpm ferrum:placement-viewer`
- `createSceneInstanceHandleRegistry(...)`를 연결해 selected id ↔ entity handle 확인
- hover/click picking: TS authoring instance bounds cache 우선, 필요 시 click 시점의 low-frequency Rust point query만 사용
- 선택/호버 표시: DOM overlay 또는 debug-line buffer로 bounding box, id label, role badge 표시
- viewer object `state()`/`selectInstance(id)`/`exportPatch()` 제공
- optional smoke hook `window.__ferrumPlacementViewer.state()` 노출
...
- 신규: browser smoke에서 `selectedInstanceId`, `pointerWorld`, canvas nonblank 단언
- `pnpm --filter @ferrum2d/ferrum-web test`
- `pnpm validate:public-api-surface`
- `pnpm validate:data-scene-authoring`
...
- `createScenePlacementPatchStore(...)` public export와 `ScenePlacementSaveAdapter` interface
- 오버레이 UI: 선택 패널, id/prefab/variant/role 표시, x/y/scale/rotation 입력, snap toggle, duplicate id diagnostic
- 이동 UX: drag, arrow-key nudge, numeric input, snap-to-grid, cancel/revert draft
- 이름 변경 UX: explicit id rename, 중복 id 차단, binding reference migration preview
- patch model: `ScenePlacementPatch`로 add/update/rename/remove 후보를 표현
- merge model: 소유 키만 갱신하는 저장기
...
- 저장 adapter:
  - 기본: patch export를 agent가 적용
  - opt-in: host-provided save adapter 또는 File System Access API
...
- rename이 필요한 binding reference 후보를 누락하지 않는지 단언하는 unit test
- save adapter를 만들 경우 allowlist/env opt-in이 없으면 저장이 실패하는 보안 테스트
- `pnpm validate:public-api-surface`
...
- inspector state 또는 patch export를 입력으로 받아 agent가 `behaviorRecipes`와 binding만 수정하는 recipe 작성 지침
- selected id 기반 behavior 적용 후 replay/hash/report가 drift를 잡는 fixture
...
- viewer host 위치: engine repo에는 `examples/placement-viewer`를 둘지, create-game template에는 `ferrum:placement-viewer` route/script를 둘지?
...
1. Slice 3A로 `PlacementViewport`와 screen↔world/snap 단위 테스트를 먼저 만든다.
2. Slice 3B로 `@ferrum2d/ferrum-web/authoring`에 `createScenePlacementViewer(...)` public API를 추가한다.
3. Slice 3C로 `examples/placement-viewer` 또는 create-game viewer route를 만들고 `pnpm dev:placement-viewer` 실행 경로를 추가한다.
4. Slice 3D로 hover/click picking과 `ScenePlacementViewer.state()` inspector를 browser smoke로 고정한다.
5. Slice 4는 write-disabled draft/patch export부터 시작하고, save adapter는 allowlist/env opt-in으로 확장한다.
6. agent 기능 부착 replay(Slice 5)를 추가해 placed id 기반 behavior 적용을 end-to-end로 고정한다.
7. `World` 장르 누수 정리(Slice 6)는 토대 안정화 후 별도 task로 진행한다.

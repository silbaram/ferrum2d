...
- **활성 후보**: fragment hierarchy v2 UX, scene-specific compatibility API, desktop packaging/GUI와 image dimension metadata의 release 범위를 결정한다.
...
- desktop wrapper packaging/GUI 수동 검증과 image dimension metadata 고도화를 product release 범위에 넣을지.
...
3. desktop packaging/GUI와 image dimension metadata는 release 포함 여부를 승인받은 뒤 별도 task로 분리한다.

...
- 현재 구현 상태는 다음과 같다.
  1. **정식 Scene Placement Authoring Viewer v1 완료**: UI가 위치 키를 선택/수정하고 agent-owned behavior를 보존하는 patch/export 흐름을 제공한다.
  2. **agent 기능 부착 루프 검증 완료**: 배치된 id에 behavior를 붙이는 dry-run/report/smoke 기준을 고정했다.
  3. **`World` 장르 누수 정리 완료**: `World.player` → marker/tag, `primaryActor`/`nearestPrimaryActor` generic target, `GameplayPrefabRegistry` 네이밍, `player_entity()` accessor 제거를 완료했다. `player`/`nearestPlayer`는 authoring 호환 alias로 유지한다.
- 남은 작업은 핵심 enabler가 아니라 editor UX와 release policy 후속이다. 새 instance id 자동 부여/rename migration UX, starter-scene public `player` 명칭 정책, File System Access API/desktop wrapper 같은 저장·배포 고도화가 다음 후보다.
...
| spawn 정의(=prefab 본체) | `EntityTemplate` (sprite 크기 + collider shape/size + frame + animation) | `crates/ferrum-core/src/world/templates.rs` |
...
- **명시 id 자동 부여 UX는 후속.** Slice 2에서 `requireExplicitInstanceIds` 검증과 registry는 추가됐고, Slice 4D에서 add/rename/remove patch와 중복 id 차단은 들어갔다. 다만 배치 UI가 새 instance id를 어떤 규칙으로 자동 제안하고 rename migration을 어떤 확인 UX로 보여줄지는 editor-only 후속으로 남긴다.
- **primary actor 호환 경로 완료.** 범용 `World`의 내부 단일 플레이어 저장소는 `primary_actor`로 중립화했고, engine-owned reserved `GameplayTags` marker와 동기화한다. movement authoring은 `primaryActor`/`nearestPrimaryActor` generic target을 canonical 표면으로 쓰며 `chase`/`seekTarget` 생략 target도 `primaryActor`로 정규화한다. `player`/`nearestPlayer`는 legacy alias로 유지한다. prefab registry/registration 계층은 `GameplayPrefabRegistry`로 중립화했고, `World`의 `player_entity()` 호환 accessor는 제거했다.
- **built-in scene 이중 구조는 제품 경계로 유지.** Shooter/Breakout/Platformer는 하드코딩 Rust scene이고, 제네릭 Data Scene(`useDataScene()`)은 별도 generic ECS tick이다(`engine/scenes.rs`). 배치 UI + agent 흐름은 본질적으로 **Data Scene** 경로이며, starter-scene public `player` 명칭 제거/alias/deprecation은 별도 release policy로 판단한다.
...
| `sceneComposition.prefabs[].props.components` | 공유 prefab catalog | typed sprite/collider/template descriptor |
...
| 개념 | 위치 | 성격 |
| --- | --- | --- |
| ① `props.components` typed schema + resolver diagnostic | TS authoring | 완료(핵심 토대) |
| ② 제네릭 prefab catalog + spawn resolver (`props.components` → `World`) | Rust core(`EntityTemplate` 승격) + TS authoring | 완료(`components.template` catalog 포함) |
| ③ package public data-scene spawn facade/default target | TS facade + 낮은 빈도 Wasm API | 완료(`createDataSceneRuntimeTarget(...)`) |
...
| ④-1 Scene Placement Authoring Viewer | `@ferrum2d/ferrum-web/authoring` + viewer host | 완료(v1 정식 제품 surface) |
| ⑤ 비충돌 save/merge (UI 소유 키만 갱신) | TS editor-only | 완료(후속: 자동 id 제안 UX) |
| ⑥ selection/picking (screen point → instance) | TS editor-only (+ 기존 point query) | 완료(v1은 TS bounds cache) |
| ⑦ de-genre `World` (`player`→tag, 네이밍) | Rust core | 완료(legacy alias 유지) |
...
- 예제 앱 수동 확인 체크리스트
- headless 좌표 변환 단위 테스트(완료)
- browser smoke에서 `selectedInstanceId`, `hoveredInstanceId`, `pointerWorld`, canvas nonblank 단언(완료, `pnpm smoke:placement-viewer`)
- `pnpm --filter @ferrum2d/ferrum-web test`
- `pnpm validate:public-api-surface`
- `pnpm validate:data-scene-authoring`
...
- 새 instance 생성 시 explicit id add API와 중복 id 방지(완료, 자동 naming UI는 후속)
...
- 배치+behavior fixture의 결정론 검증(완료, selected id 기반 unit regression + variant expected drift check + replay manifest link)
...
- `props.components` v1은 inline descriptor와 `components.template` catalog reference를 병행한다. catalog entry가 다시 template을 가리키는 recursive template은 허용하지 않는다.
- texture authoring id는 v1에서 `engine.textureId(name)` 또는 `CreateDataSceneRuntimeTargetOptions.textureId(name)`로 해석한다. scene-authoring 문서 내부 asset registry는 후속 후보로 둔다.
- generated fallback id(`fragment.index`)는 read-only/import 호환용으로만 남기고, UI/agent 타겟팅 apply 경로는 `requireExplicitInstanceIds: true`를 사용한다.
- v1 편집 단위는 단일 fragment 평면 편집이다. 중첩 fragment 절대→상대 transform 역변환은 제외 범위로 유지한다.
- preview는 `useDataScene()` 라이브 spawn을 기본값으로 한다. 경량 editor-only renderer는 필요 시 headless math fallback 후보로만 둔다.
- spawn 정의 소스는 기존 `EntityTemplate`/`PrefabEntitySpawnRequest` 경로를 재사용하고, TS facade가 `props.components`를 numeric Wasm 인자로 컴파일한다.
- viewer host 위치는 engine repo의 `examples/placement-viewer`와 generated consumer project의 `npm run ferrum:placement-viewer`로 확정했다.
- 저장은 patch export와 host-provided save adapter를 v1 기본으로 한다. File System Access API는 optional enhancement다.

### 후속 질문

- placement viewer가 새 instance id를 자동 제안하는 naming 규칙과 rename migration 확인 UX를 어떻게 둘지?
- `primary_actor` marker/tag 동기화 이후 public starter-scene API의 `player` 명칭을 유지/alias/deprecate 중 어떤 release policy로 가져갈지?
- File System Access API를 공식 옵션으로 둘지, patch export/host adapter만으로 v1을 유지할지?
- desktop wrapper 요구가 생기면 Tauri 우선 spike와 Electron 비교를 별도 `deployment-roadmap.md` slice로 분리할지?

## 별도 승인 필요 범위

- full visual editor 제품화: 노드 트리, 풀 prefab editor, 멀티유저 동시 편집, behavior UI 편집은 별도 승인.
- Scene 추상화 승격 / starter-scene public `player` API 제거처럼 엔진 코어 구조·public compatibility·snapshot 계약에 닿는 변경: 별도 task 또는 이슈로 설계 합의 선행.

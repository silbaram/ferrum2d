...
`docs/planning`은 Ferrum2D의 미구현 신규 기능 후보, 승인 필요 기능, 리팩토링 후보를 관리하는 영역이다. 구현이 완료된 기능의 사용법, public contract, 검증 기준은 `docs/engine` 또는 `docs/development`로 옮기고 planning 문서에서는 삭제한다.
...
- 기능이 코드, 테스트, 문서까지 완료되면 planning 문서에서 삭제하고 확정 문서에만 남긴다.

...
Rust/Tauri 쪽은 session-local asset id를 asset folder 내 canonical path로 매핑한다. 요청 path는 반드시 현재 선택된 asset folder 하위 파일이어야 하며, `..` traversal과 symlink 탈출은 거부한다. 구현 시 Tauri v2의 custom protocol API 이름과 MIME 응답 API는 로컬 dependency 버전에 맞춰 확인한다.

### 2. Desktop asset manifest 생성

`inspect_placement_asset_folder` 응답은 현재 image list evidence만 반환한다. 다음 구현에서는 같은 검사 결과에서 placement runtime용 manifest를 파생한다.
...
따라서 다음 구현은 asset URL provider와 manifest 생성에 집중하고, WebGL2/WebGPU texture upload 자체는 재사용한다.
...
- 실제 Tauri GUI에서 native asset folder picker, custom protocol fetch, `.app` 실행, handoff 자동 sync 상태를 확인한다.
- local image의 실제 width/height metadata를 runtime/preview provider에 연결해 Add Sprite 기본 크기가 고정값에 덜 의존하도록 개선한다.

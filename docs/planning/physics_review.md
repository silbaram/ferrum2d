...
기존 리뷰 내용은 개선 후보 목록으로는 유용하지만, 모든 항목을 확정 버그로 볼 수는 없다. 초기 검토에서 Baumgarte bias 부호 변경, pulley slack, moving platform 회전 상속, KCC jump step-up, slope tangent projection은 게임감/제품 계약에 따라 의도 동작일 수 있어 회귀 fixture 또는 별도 제품 결정을 먼저 두었다. 현재 pulley slack과 moving platform rotation carry는 기존 기본값을 보존하는 opt-in 계약으로 완료됐고, KCC 기본 정책과 singular 3x3 입력 안전성도 회귀 기준과 함께 확정됐다. 남은 일반 후보는 coupled weld fallback 중복 연산과 고차수 joint graph despawn 비용 측정이다.

| 분류 | 의미 | 예시 |
| --- | --- | --- |
| 확정 개선 후보 | 코드 구조상 한계가 확인됐고, 테스트를 붙여 안전하게 진행 가능 | coupled weld fallback의 constraint context 중복 생성 |
...
현재 물리 개선 planning의 Slice 0-5는 완료 상태다. singular matrix 방어도 후속 B에서 완료했으며, 다음 일반 개선 후보는 아래 solver 중복 연산과 고차수 despawn profiling이다. 각각 회귀 fixture와 성능 기준을 별도 task에서 확정한다. Deferred 항목은 명시적 승인 없이는 착수하지 않는다.

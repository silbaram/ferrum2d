...
| Slice 3 | 진행 중 | joint rotational coupling 개선 | local anchors, effective mass angular term, weld coupling |
...
- 남은 Slice 3 작업: Spring soft constraint migration 검토, Weld anchor gap position correction의 angular coupling fixture/개선, Slice 3 전체 smoke matrix 실행.
...
현재 바로 이어가기 좋은 다음 작업은 **Slice 3 joint rotational coupling 설계 확정**이다. Slice 2의 KCC step offset early-out, KCC entity candidate provider, sleep/wake manifold rebuild early-out, runtime budget/smoke metric 연결은 적용 및 검증 완료 상태다.

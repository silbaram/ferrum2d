...
현재 물리 개선 planning의 Slice 0-5는 완료 상태다. 다음 물리 확장은 Deferred 항목 중 별도 승인된 항목이나 새 planning 문서로 다룬다.

## 신규 발견 잠재적 개선 사항 (2026-06-17 추가)

Slice 0-5 완료 이후 코드 심층 리뷰에서 추가로 확인된 개선 및 보완 계획이다.

### 1. 엔티티 디스폰(Despawn) 시 조인트 리소스 누수 방지
- **문제**: `distance_joint_should_break` 및 `weld_joint_should_break`는 엔티티가 디스폰되어 `constraint_context` 생성에 실패(`None`)할 때 `false`를 반환하므로, 솔버 루프에서 해당 조인트가 지워지지 않고 누수될 위험이 있음.
- **방향**: 컨텍스트 획득 실패 사유가 엔티티 부재인 경우, 명시적으로 `true`를 반환하여 조인트를 클리어하도록 예외 제어 추가.

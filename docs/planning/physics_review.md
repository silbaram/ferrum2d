...
기존 리뷰 내용은 개선 후보 목록으로는 유용하지만, 모든 항목을 확정 버그로 볼 수는 없다. 초기 검토에서 Baumgarte bias 부호 변경, pulley slack, moving platform 회전 상속, KCC jump step-up, slope tangent projection은 게임감/제품 계약에 따라 의도 동작일 수 있어 회귀 fixture 또는 별도 제품 결정을 먼저 두었다. 현재 pulley slack과 moving platform rotation carry는 기존 기본값을 보존하는 opt-in 계약으로 완료됐고, KCC 기본 정책과 singular 3x3 입력 안전성도 회귀 기준과 함께 확정됐다. coupled weld fallback의 constraint context 중복 생성도 순차 보정의 World mutation을 보존하는 범위에서 정리했다. 남은 일반 후보는 고차수 joint graph despawn 비용 측정이다.

| 분류 | 의미 | 예시 |
| --- | --- | --- |
| 완료된 확정 개선 | 코드 구조상 한계가 확인됐고, 테스트를 붙여 안전하게 완료 | coupled weld fallback의 mutation-aware constraint context 재사용 |
| 재현·측정 필요 | 가능성은 있으나 버그나 성능 병목으로 단정하면 위험 | 고차수 joint graph despawn 비용 |
| 제품 결정 완료 | 기존 기본값을 보존하면서 opt-in public 계약을 확정함 | continuous revolute limit, pulley slack, moving platform rotation carry |
...
| 후속 A | 완료 | solver hot path 정리 | coupled weld fallback의 mutation-aware constraint context 재사용 |
| 후속 B | 완료 | 수치 안전성 보강 | finite reciprocal mass/inertia gate, singular 3x3 safe fallback fixture |
...
현재 물리 개선 planning의 Slice 0-5는 완료 상태다. singular matrix 방어와 coupled weld fallback context 재사용도 후속 A/B에서 완료했으며, 다음 일반 개선 후보는 고차수 despawn profiling이다. 회귀 fixture와 성능 기준은 별도 task에서 확정한다. Deferred 항목은 명시적 승인 없이는 착수하지 않는다.

## 후속 개선 후보 (2026-06-17 추가, 2026-07-15 갱신)
...
### 2. Coupled Solver의 Fallback 시 중복 연산 최적화 — 완료 (2026-07-15)
...
- **조치**: context-aware prismatic position helper를 추가하고 Weld position solve가 최초 context를 coupled/decoupled 경로에 참조로 전달한다. fallback의 linear/limit 보정은 body transform과 rotation을 바꿀 수 있으므로, 같은 context를 무조건 재사용하지 않고 실제 보정이 적용된 뒤에만 다음 단계용 context를 갱신한다.
- **회귀 기준**: 이미 정렬된 Weld position solve의 context 생성은 4회에서 1회로 줄이고, singular coupled solve에서 linear/limit/angular fallback이 순차 mutation을 일으키는 fixture는 context를 정확히 3회 생성하면서 finite transform/rotation을 유지한다.

### 3. 수치적 특이성(Singular Matrix) 안전성 보강 — 완료 (2026-07-15)
- **문제**: static/kinematic body의 무한 관성은 `inverse_inertia = 0`으로 표현되고 non-finite dynamic mass/inertia 입력은 경계에서 거부된다. 다만 현재 양수 검증은 최솟값을 제한하지 않으므로 극소 양의 mass/inertia의 역수가 non-finite가 될 수 있고, 큰 유한 질량 행렬 성분의 곱셈도 Determinant 계산에서 f32 오버플로우/언더플로우를 만들 수 있다.
- **조치**: `RigidBody::dynamic(...)`과 `with_inertia(...)`는 역수가 유한하지 않은 극소 양수 입력을 각각 기본 mass 또는 기존 inertia로 보정한다. Wasm mass-property setter와 body snapshot restore는 같은 입력을 mutation 없이 거부한다. Weld 3x3 solver는 mass matrix와 RHS 성분을 determinant 계산 전에 검사하고, non-finite 성분·determinant·correction이면 기존 개별 constraint fallback을 사용한다.
- **회귀 기준**: 최소 subnormal mass/inertia의 constructor 보정, Wasm setter 거부, snapshot restore 무변경 거부, non-finite Weld mass matrix의 finite transform/rotation 보존을 Rust unit test로 고정한다.

...
현재 개발 단계는 **MVP 개발 완료, 상용제품 기능 개발** 이다. `examples/minimal-game`은 제품용 starter runtime 흐름을 검증하고, `examples/topdown-shooter`는 게임 기능과 회귀 시나리오를 검증한다.
...
Physics v3 기반은 shooter 전용 layer 의존을 줄이는 범용화 작업이다. `CollisionMask`와 `CollisionFilter`는 entity별 category/mask bitset을 제공하고, `CollisionSystem::build_mask_pairs(...)`와 `build_swept_mask_pairs(...)`는 layer enum이 아닌 category bitset으로 pair를 만든다. `point_query(...)`, `aabb_query(...)`, `raycast(...)`는 mask 기반 world collider query를 제공한다. `AabbContact`와 `CollisionContact`는 current-frame overlap의 normal/penetration을 제공하고, `SweptAabbContactHit`는 swept time-of-impact와 entry normal을 함께 제공한다. `PhysicsSystem::move_and_slide(...)`와 `move_and_slide_with_tilemap(...)`는 solid collider와 tile obstacle에 대한 kinematic movement를 처리하되 mass/force/impulse는 도입하지 않는다. `FixedTimestep`은 opt-in fixed update accumulator와 render interpolation alpha를 계산하고, `PhysicsCounters`는 fixed step과 kinematic collision 후보/히트 수를 누적해 `FrameState.physics`로 노출한다. `CollisionEventTracker`는 AABB pair enter/stay/exit를 `CollisionEvent` u32 buffer로 기록하고 shooter gameplay 충돌은 despawn 전에 `hit` event를 기록한다. TypeScript는 `decodeCollisionEvents(...)`로 필요한 경우에만 object 배열로 변환한다.
...
| `tile checks` | count | 현재 frame에서 kinematic tile collision 후보로 검사한 tile 수 |
...
- `FerrumEngine`, `FrameState`, `PhysicsFrameStats`, collision event 타입, `Renderer`, `RendererStats`, `AssetManifest`, `ShooterGameSpec` import 경로

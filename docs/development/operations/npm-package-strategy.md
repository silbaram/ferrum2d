...
기본 PR/main CI의 validate job은 `pnpm package:check`를 실행해 세 package tarball allowlist, Wasm artifact, create-game template matrix, agents installer package contract를 함께 검증한다. 실제 consumer 프로젝트 설치/빌드 matrix인 `pnpm package:consumer-smoke`는 tag 또는 명시적 workflow dispatch에서 실행한다.

...
`ferrum:report`는 `ferrum2d.consumer.project.report` JSON envelope로 package dependency, generated file, internal import, root aggregate import를 요약하고 최상위 `recommendedCommands`를 제공한다. `ferrum:smoke`는 생성 프로젝트 안에서 validate/build를 실행하는 기본 소비자 검증 gate다.

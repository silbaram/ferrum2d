...
`@ferrum2d/create-game` 검증은 실제 템플릿 생성 결과의 `package.json`, public API import, AI 개발 하네스 스크립트, 필수 starter 파일을 확인한다. `@ferrum2d/agents` 검증은 consumer skill/agent frontmatter, Claude wrapper, Gemini custom command 규칙, dry-run 무변경 동작, 실제 설치 결과를 확인한다.

...
- 전체 게임 루프 Worker 이전, editor, multiplayer, complex physics는 현재 제품 범위가 아니다.
...
- publish하지 않은 draft release에는 npm 설치 명령 대신 local tarball 확인 경로를 적는다. 기본 rehearsal artifact는 `artifacts/consumer-smoke-release-local/consumer-smoke-report.json`이다.

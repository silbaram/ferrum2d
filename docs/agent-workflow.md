...
| `docs-agent` | `README.md`, `docs/*` | public API, workflow, 제품 문서 동기화 |
...
별도 variant 파일을 검증할 때는 `node scripts/validate-game-spec.mjs path/to/game.json`와 `pnpm smoke:headless path/to/game.json`처럼 대상 경로를 넘긴다.
...
상용제품 기능 개발 단계에서는 다음을 별도 설계/승인 없이 subagent에게 맡기지 않는다.

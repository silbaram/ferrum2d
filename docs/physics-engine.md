...
현재 구현은 AABB와 tilemap AABB obstacle 중심이다. circle, capsule, polygon, one-way platform은 post-MVP 설계 후 추가한다.
...
이 항목들은 2D 게임 물리엔진에는 흔하지만, Ferrum2D MVP 완료 이후 안정화 범위에서도 크기와 위험이 크다. 구현이 필요하면 Physics v3 이상 milestone으로 분리한다.

...
이 문서는 2D 게임에서 일반적으로 쓰이는 물리엔진 기능을 Ferrum2D 관점으로 정리한다. 현재 저장소의 구현 범위는 [Physics v2 범위](physics-v2.md)와 [Physics v3 기반](physics-v3.md)을 기준으로 하며, 아래 항목은 전체 기능 목록과 단계별 후보를 구분하기 위한 기준이다.
...
| AABB contact | 현재 frame overlap의 normal과 penetration 계산 | `crates/ferrum-core/src/collision.rs` |
| Physics query | point/AABB/raycast 기반 world collider 조회 | `crates/ferrum-core/src/collision.rs` |
...
| Collision events | AABB pair enter/stay/exit lifecycle, gameplay hit event, Wasm u32 buffer decode | `crates/ferrum-core/src/collision_event.rs`, `packages/ferrum-web/src/collisionEventDecoder.ts` |
| Shooter collision | bullet/enemy hit, player/enemy game-over 충돌 연결 | `crates/ferrum-core/src/shooter_scene.rs` |
| Tilemap obstacle | `collision: true` tile layer를 정적 AABB 장애물로 사용하고 collider AABB 주변 tile만 검사 | `crates/ferrum-core/src/tilemap.rs` |
...
현재 구현은 AABB overlap, swept AABB hit time, AABB contact normal/penetration이다. 여러 contact point를 갖는 manifold는 아직 없다.
...
현재는 collision pair query, point query, AABB query, raycast를 지원한다. shape cast와 nearest body query는 아직 없다.
...
현재는 `collision: true` layer의 양수 tile을 player/enemy AABB 장애물과 navigation obstacle로 사용한다. dynamic collider가 겹치거나 swept path가 지나가는 tile 범위만 검사하지만, tile AABB merge와 slope tile은 아직 없다.
...
현재는 shooter scene 내부에서 pair query 결과를 직접 소비하고, 범용 관측용으로 `CollisionEventTracker`가 current-frame AABB pair의 enter/stay/exit를 `CollisionEvent` u32 buffer에 기록한다. 충돌 처리 직후 despawn되는 shooter bullet/enemy와 player/enemy 충돌은 `hit` event로 같은 buffer에 남긴다. TypeScript는 `FrameState.collisionEventBuffer`를 기본으로 읽고, object 배열이 필요한 경우 `includeCollisionEvents` 또는 `decodeCollisionEvents(...)`를 사용한다. entity별 JS/Wasm callback, pre/post-solve, damage payload는 아직 없다.
...
현재 Rust core에는 `PhysicsCounters`가 있으며 fixed step, kinematic hit, solid/tile 후보 검사 수를 누적한다. TypeScript는 이 값을 `FrameState.physics`로 읽고 Top-down Shooter DebugOverlay는 `fixed steps`, `kinematic hits`, `tile checks`, `collision events`를 표시한다. contact point/normal debug draw와 broadphase 시각화는 아직 없다.
...
| 단계 | 후보 | 목적 |
| --- | --- | --- |
| Physics v3 | collision mask bitset, AABB contact, point/AABB/raycast query, kinematic move-and-slide, tile candidate query, opt-in fixed timestep, physics counters/API, collision event lifecycle | 현재 shooter 구조를 유지하면서 query와 관측성을 확장 |
| Physics v4 | circle collider, tile AABB merge | 더 다양한 2D gameplay와 tilemap 성능 개선 |
| Physics v5 | one-way platform, ground detection, moving platform carry | platformer 기반 마련 |
...
Physics v3 중 collision mask bitset, AABB contact, point/AABB/raycast query, world/tilemap kinematic move-and-slide, tile candidate query, opt-in fixed timestep runtime, physics counters/API 노출, collision event lifecycle과 gameplay hit event는 [Physics v3 기반](physics-v3.md)으로 완료했다. 남은 후보는 circle collider, shape query, damage payload, contact debug draw다.

...
12. For placement-driven behavior edits, use `gameplayAuthoring.sceneAuthoring.summary.placementAuthoring.instances[]` or the placement viewer handoff JSON to confirm the selected `instanceId`, role, prefab/ObjectDefinition, behavior profiles, `behaviorBindings[].recipeId`/`bindingPath`/`behaviorRecipePath`, command types, visual/collider summary, and runtime entity handle before editing behavior data.
...
- When the viewer creates a reusable ObjectDefinition, keep `addObjectDefinition` changes in `sceneComposition.prefabs[]` and keep ObjectDefinition-backed instance placement in `sceneComposition.fragments[].instances[]`.
- Treat ObjectDefinition `props.components.visual`/`collider`/`layer` as reusable object template data. Do not put behavior recipe bodies inside a placement-only ObjectDefinition patch unless the task explicitly asks for behavior attachment.
...
- Treat `updateBehaviorBinding` as a Behavior Binding reference patch. It can attach or detach an existing recipe id, but it is not a behavior recipe body edit.
- Use `behaviorBindings[].recipeId`, `bindingPath`, and `behaviorRecipePath` from the report or handoff as evidence for the agent-owned behavior target before editing `behaviorRecipes.entities`.
...
- If the selected instance came from a draft ObjectDefinition, confirm the resolved `prefab`, `visual`, and `collider` in the handoff/report before attaching behavior to that prefab or to a specific instance.

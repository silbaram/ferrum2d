...
- Rust `World` 기반 entity, transform, velocity, sprite, offset-aware/enabled AABB/circle/capsule/oriented box collider 저장
- AABB 충돌 판정과 bullet/enemy, player/enemy 충돌 처리
- sweep-and-prune broadphase, collision filter, point/AABB/circle/oriented-box/capsule/convex-polygon shape query, AABB/circle/oriented-box/capsule/convex-polygon shape cast, raycast/segment-cast/nearest body query, local offset 및 enable/disable AABB/circle/capsule/oriented-box/convex-polygon collider, 저장형 oriented box collider pair/query/raycast/shape-cast target/nearest body/contact/face-clipped 2점 manifold/rigid CCD, 저장형 capsule collider overlap/query/raycast/shape-cast/contact/side chord manifold, 저장형 convex polygon collider pair/query/raycast/shape-cast/moving shape-cast/nearest body/contact/manifold/rigid CCD, AABB/circle/capsule/oriented-box/convex-polygon contact, swept AABB 기반 빠른 bullet/enemy 터널링 방지
- world collider와 tilemap obstacle 대상 swept AABB 기반 kinematic move-and-slide, entity/tilemap one-way platform, Game Spec/Tiled/LDtk one-way tile metadata, moving platform carry, tilemap nearest obstacle query, runtime tile collision refresh, tilemap rect edit 기반 destructible terrain prototype, platformer ground probe/controller, coyote time, jump buffering, step offset
- Rust core opt-in dynamic rigid body, box/circle/capsule/oriented-box/convex-polygon density 기반 mass/inertia helper, force/impulse accumulator, restitution/friction/surface velocity/density material, material Baumgarte/position correction/slop tuning, configurable restitution threshold, collider material override, body enable/disable, substep rigid body solver, contact/joint island metrics와 sleep/wake propagation, island별 solver scheduling, translational/angular impulse solver, configurable contact Baumgarte stabilization, opt-in contact split impulse, split position impulse, circle/circle 단일점 scalar solve, 2점 AABB contact manifold, 단일점 circle 및 AABB/oriented-box/convex-polygon face chord 2점 circle contact manifold, oriented box face-clipped 2점 contact manifold, capsule side/endpoint/비평행/crossing curve/face arc clipping/circle side chord 2점 contact manifold, convex polygon face-clipped/round-shape/face arc clipping contact manifold, AABB/capsule/oriented-box/convex-polygon 2점 contact block solver와 arc-clipped/crossing capsule/circle-polygon face/capsule-circle side manifold block solver 회귀, persistent warm start cache, sleep/wake 자동화, dynamic AABB CCD 1차, dynamic-dynamic AABB CCD 1차, dynamic AABB/circle/capsule/oriented-box/convex-polygon CCD 1차와 dynamic convex polygon target matrix 회귀 검증, 제한 반복 multi-TOI CCD 1차, distance/rope/spring/revolute/prismatic/weld/gear joint constraint, distance/rope/spring/revolute/prismatic/weld/gear joint break threshold, revolute angle limit/motor, prismatic translation limit/motor, angular body integration
- opt-in fixed timestep runtime과 Rust core pause/resume utility, physics counters/API, collision pair metrics, Web rigid body authoring/control/material/tuning/snapshot API, Web collider offset/enable/material control/snapshot API, Web joint authoring/control/snapshot API, Web opt-in rigid body step stats API, rigid body post-solve contact impulse snapshot과 Web bulk query API, collision event buffer with damage payload and AABB/circle/capsule/oriented-box/convex-polygon trigger lifecycle event kinds, Web point/AABB/circle/oriented-box/capsule/convex-polygon body query API, Web raycast/segment-cast body query API, Web AABB/circle/oriented-box/capsule/convex-polygon shape-cast body query API, Web tilemap raycast/segment-cast 및 AABB obstacle shape-cast/contact/manifold API, Web body contact/manifold query API
...
- JSON Game Spec 기반 world/player/enemy/weapon/prefab/prefab collider/behavior/camera/atlas frame/atlas animation/tilemap/audio 조정
- `collision: true` tilemap layer 기반 player/enemy merged AABB 장애물, runtime cell/rect 변경 cache refresh, chase enemy Navigation Grid v1
- Horizontal sprite sheet, idle/move state, atlas frame sequence 기반 player/enemy/bullet sprite animation
...
- Physics Spec authoring 검증용 `examples/physics-sandbox` 예제와 browser smoke
...
패키지 공개 후보의 entrypoint, CLI 템플릿, agent/skill 템플릿, 포함 파일, generated Wasm artifact, 실제 `pnpm pack` tarball 구성을 확인한다.

```bash
pnpm package:check
```

릴리스 후보의 changelog, beta version, Git tag 규칙을 확인한다. tag push CI에서도 같은 검증이 실행된다.

```bash
pnpm release:check
```

GitHub Pages demo/docs artifact는 전체 build 이후 생성한다.
...
데모/문서 사이트 배포 절차는 [docs/development/operations/demo-deploy.md](docs/development/operations/demo-deploy.md)에 고정한다. npm beta package 절차는 [docs/development/operations/npm-release.md](docs/development/operations/npm-release.md)에, GitHub Release 본문 구조는 [docs/development/operations/release-notes-template.md](docs/development/operations/release-notes-template.md)에 고정한다. 실제 publish는 `private: true` 해제와 npm 권한 확인이 승인된 뒤 별도로 수행한다.
...
MVP 개발 완료 이후 상용제품 기능 개발 단계에서는 위 자동 검증과 Top-down Shooter manual smoke check를 회귀 기준으로 유지한다. 자세한 로컬/CI 검증 차이와 브라우저 수동 확인 절차는 [docs/development/quality/smoke-check.md](docs/development/quality/smoke-check.md)를 따른다.
...
Top-down Shooter 수동 점검 기준은 `docs/development/quality/topdown-shooter-smoke-checklist.md`를 따른다.
...
Product Beta Asset pipeline v2의 현재 범위는 Aseprite JSON metadata, Tiled JSON tilemap, LDtk JSON tilemap을 Ferrum2D Game Spec 조각으로 변환하는 authoring helper다. Tiled tile custom property `oneWayPlatform: true`와 LDtk tileset `customData` JSON `{"oneWayPlatform":true}`는 Game Spec `tilemap.tiles.*.oneWayPlatform`으로 변환된다. 변환은 TypeScript에서만 수행하고 Rust/Wasm render command ABI는 바꾸지 않는다.
...
LDtk project JSON도 embedded level 또는 앱이 미리 로드해 `externalLevels`로 넘긴 `.ldtkl` level의 Tiles/AutoLayer tile instances를 같은 `atlas`와 `tilemap` Game Spec 조각으로 변환할 수 있다. `collisionLayerNames`에 포함된 raw `IntGrid`는 렌더링 없는 `collisionOnly` tilemap layer로 변환된다.
...
세부 API 계약은 [docs/engine/public-api.md](docs/engine/public-api.md)를 따른다.
...
Top-down Shooter 예제는 `json.game`을 Game Spec으로 해석한다. TypeScript가 JSON을 검증하고 기본값을 채운 뒤 Rust `Engine.set_shooter_resolved_config(...)`, `Engine.set_shooter_animations(...)`, `Engine.set_shooter_camera_preset(...)`, `Engine.set_shooter_atlas_frame(...)`, `Engine.set_shooter_atlas_animation(...)`, `Engine.set_shooter_prefab_collider(...)` 또는 shape별 `Engine.set_shooter_prefab_*_collider(...)`, `Engine.set_shooter_tile(...)`, `Engine.set_shooter_tile_slope(...)`, `Engine.set_shooter_tile_one_way_platform(...)`, `Engine.set_shooter_tilemap_layer(...)`, `Engine.set_shooter_wave(...)`, `Engine.set_shooter_audio_policy(...)`에 숫자형 설정과 typed array를 전달한다. `prefabs.*.animation`은 horizontal sprite sheet의 frame count/fps, `idle`/`move` state row, 또는 `animation.atlas` frame sequence를 설정하며, Rust가 매 프레임 UV를 갱신한다. `prefabs.*.collider`는 player/enemy/bullet AABB/circle/capsule/oriented-box/convex-polygon collider shape, local offset, enable/trigger flag, optional material override를 설정한다. `atlas.frames`와 `prefabs.*.frame`은 texture name/id, normalized UV rect, frame size를 검증해 static atlas frame을 prefab에 연결한다. `animation.atlas`는 같은 texture/size를 가진 atlas frame sequence를 prefab animation에 연결한다. `tilemap`은 atlas frame 기반 정적 tile layer를 렌더링 command로 추가하고, `collision: true` layer의 양수 tile은 player/enemy 이동을 막는 AABB 장애물이자 chase enemy navigation 장애물로 사용한다. Runtime 단일 cell 변경은 `FerrumEngine.setShooterTilemapTile(...)`, 직사각형 cell 묶음 변경은 `FerrumEngine.setShooterTilemapTilesRect(...)`로 수행하고 collision layer 변경은 Rust tile obstacle cache를 즉시 refresh한다. `tilemap.tiles.*.oneWayPlatform: true` tile은 위에서 내려오는 movement/ground probe만 막는 tilemap one-way platform으로 전달된다. `collisionOnly: true` layer는 렌더 tile definition 없이 raw collision grid를 전달할 때 사용한다. `enemies.presets`와 `enemies.waves`는 wave timeline을 구성하고 Rust가 wave 진행과 spawn count를 소유한다. `enemies.orbit`은 orbit behavior의 목표 반경과 보정 폭을 숫자 설정으로 전달한다. `audio`는 Web Audio bus volume과 Rust audio event volume/pitch를 조정한다. `camera.preset`은 follow, dead-zone, look-ahead, shake를 지원하며 Rust가 카메라 위치를 계산한다.

프레임 렌더링에서는 `FrameState.renderCommandBuffer`를 사용한다. `FrameState.physics`는 fixed timestep/kinematic/collision pair/collision event frame stats를 담고, `FrameState.collisionEventBuffer`는 collision enter/stay/exit/hit과 trigger-enter/trigger-stay/trigger-exit raw buffer를 제공한다. `decodeCollisionEvents(...)`가 만드는 `CollisionEventView.damage`는 bullet/enemy hit에 적용된 bullet damage이며, lifecycle event와 player/enemy game-over hit는 `0`이다. 저수준 physics API는 rigid body authoring/control/material/tuning/snapshot, collider offset/enable/material control/snapshot, joint authoring/control/snapshot, body overlap/raycast/body shape-cast, tilemap raycast/segment-cast와 AABB obstacle shape-cast/contact/manifold, current-frame body contact/manifold, opt-in rigid body step stats, 마지막 rigid body step contact impulse snapshot을 public API로 제공한다. `FrameState.physicsDebugLineBuffer`는 broadphase AABB proxy bounds와 contact normal debug line raw buffer를 제공하지만, Rust line 생성은 `enablePhysicsDebugLines`, `includePhysicsDebugLines`, 또는 runtime의 `physicsDebugLines`를 켠 경우에만 수행한다. `WebGL2Renderer.renderPhysicsDebugLines(...)`와 `createFerrumRuntime(..., { physicsDebugLines: true })`는 이 buffer를 같은 frame의 debug line pass로 그린다. `FrameState.renderCommands`는 호환성 유지용 deprecated API이며 기본값으로는 빈 배열을 반환한다. command object 배열이 꼭 필요한 기존 코드는 `createEngine(..., { includeDeprecatedRenderCommands: true })`로 명시적으로 켤 수 있다.
...
`WebGL2Renderer.stats()`는 draw call, batch, sprite, render command, texture bind, texture switch 수를 반환한다. Top-down Shooter 예제는 이 값과 초당 audio event 수, physics frame stats, collision event 수를 DebugOverlay에 표시한다. 표시명과 단위 계약은 [docs/development/architecture/architecture.md](docs/development/architecture/architecture.md)의 아키텍처 기준을 따른다.
...
- Engine: [사용자 설명서](docs/engine/user-guide.md), [Public API](docs/engine/public-api.md), [Top-down Shooter Game Spec](docs/engine/topdown-shooter-game-spec.md)

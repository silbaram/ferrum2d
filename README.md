...
- JSON Game Spec 기반 world/player/enemy/weapon/prefab/behavior/camera/atlas frame/tilemap 조정
...
- WebGPU 렌더러 (실험용 scaffold만 존재, sprite 커맨드 렌더링 미구현)
- `createRenderer()`는 `preferred: "webgpu"` 선택 시 WebGPU 초기화를 시도하고 실패하면 WebGL2로 fallback한다. 기본 `console.warn` 로그를 남기며 `onFallback` 콜백으로 원인을 수집할 수 있다.
